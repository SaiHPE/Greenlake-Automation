...
Current: **v0.16.0** (2026-09-19, first non-pre-release) — the findings register is closed (SPEC-001…013; ADR 0013). The provisioning track (discovery → zoning → provision → verify → as-built) is live-verified on rack13arcus by hand and by the self-checking runner (`scripts/session.ps1`, SPEC-006; 58/58 app assertions on rc.28, 2026-09-19). G-1 closed 2026-09-17 (a tool-generated zone logged a host in). S-3 failure paths and S-5 resume seen live. SPEC-013 ESXi view read live (absent + rescan sentence). **The init track is built, NOT live-verified** — needs a factory-fresh array; say so in every status. Owed live: S-9 (iSCSI, `pending.ps1 -Iscsi`), SPEC-013 *Visible* after a rescan, the init track. G-2 deferred (SPEC-014). Releases: tag `v0.16.x` (no dash → full release) → github.com Actions builds the exe zips; `gh release create` attaches `session.ps1`/`session.cmd`; `pwsh scripts/publish_release.ps1` refreshes GHES `latest`. Shipped `.ps1` pure ASCII (LESSONS 38), TLS callback via `Add-Type -IgnoreWarnings` (LESSONS 40). Diagnostics go to the operator as paste-scripts (`ui-smoke/pending.ps1`, gitignored), not commits; no release just to retest.
