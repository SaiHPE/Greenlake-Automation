...
Current: **v0.16.0-rc.28** (2026-09-18) — the findings register is closed (SPEC-001…012; ADR 0013 one array credential per run). The provisioning track is live-verified on rack13arcus, including twice by the self-checking session runner (`scripts/session.ps1`, SPEC-006; 57/58 on rc.24). **G-1 closed 2026-09-17**: a tool-generated zone pasted as rendered logged a host into the array. Failure paths (S-3) seen 2026-09-17 via the paste-script `ui-smoke/pending.ps1` (gitignored, never committed) → D-10/D-11/Z-7 fixed in rc.27. S-5 (resume) seen live 2026-09-17. rc.28: SPEC-013 ESXi view (G-3, second column, never the verdict), X-10 run history under %LOCALAPPDATA%, P-22. Owed live: S-9 (iSCSI, `pending.ps1 -Iscsi`), SPEC-013 *ok* state after a rescan, the init track on a factory-fresh array. G-2 deferred (SPEC-014). Releases: tag `v0.16.0-rc.N` → github.com Actions builds the exe zips; `gh release create` attaches `session.ps1`/`session.cmd` standalone (the jump box cannot reach raw.githubusercontent.com); `pwsh scripts/publish_release.ps1` refreshes GHES `latest`. Shipped `.ps1` must be pure ASCII (LESSONS 38) and compile its TLS callback with `Add-Type -IgnoreWarnings` (LESSONS 40). Diagnostics go to the operator as paste-scripts, not commits; no release just to retest.
