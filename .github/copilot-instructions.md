...
Current: **v0.16.0-rc.23** (2026-09-15) — the findings register is closed (SPEC-001…012; ADR 0013 one array credential per run). The provisioning track is live-verified on rack13arcus, including twice by the self-checking session runner (`scripts/session.ps1`, SPEC-006; 38/39). Owed live: S-3/S-5/S-6/S-9, G-1 (zoning apply with the training team), the init track on a factory-fresh array, and one screenshot pass for the rc.19–rc.23 UI changes. Releases: tag `v0.16.0-rc.N` → github.com Actions builds the exe zips; `gh release create` attaches `session.ps1`/`session.cmd` standalone (the jump box cannot reach raw.githubusercontent.com); `pwsh scripts/publish_release.ps1` refreshes GHES `latest`. Shipped `.ps1` must be pure ASCII (LESSONS 38) and compile its TLS callback with `Add-Type -IgnoreWarnings` (LESSONS 40).
