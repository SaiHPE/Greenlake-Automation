...
### S-14 — 2026-09-17 10:30, rc.26 + `pending.ps1` (paste-script, not shipped): **33 PASS / 1 FAIL**

`pending.ps1` drove the items the session runner does not cover: the failure paths (S-3), an iSCSI
feasibility read (S-9), a G-1 context run, and the restart half of S-5. The FAIL was the script's own
null read on an empty 409 body (PowerShell 5.1 leaves a 4xx response stream at its end — `HTTP 409:  `
in S-13 too; both scripts now read `ErrorDetails.Message`), which also skipped S-5 — rerun owed.

S-3, first time seen: wrong array login (SSH + WSAPI), wrong verify override, wrong F1 switch login,
wrong vCenter login, all with made-up usernames. No crash, no password in any of 60 responses, every
transport names its endpoint. Three sentences were wrong and went to **rc.27**: **D-10** (array login
refused → *Discovery complete — 0 array port(s)*, step READY; now the step's error, run
`retryable_failure`, *fix the sheet and start a new run*), **D-11** (vCenter fault property dump; now
*Login failed for user@host — check the vCenter username/password on the sheet (…)*), **Z-7** (no array
ports → *Zoning needs 0 zone(s) … nothing can be provisioned*; now one error). Also live: D-7 (1
logged-in vCenter host with its ports on the row), D-8, the `/runs` list behind *Open another run*.
S-9: 4 iSCSI target ports READY, 10 hosts already on iSCSI — feasible on this array.

### G-1 — 2026-09-17 12:19: the tool's zone logs a host in

Context run `d45a12f6`. Before: `localhost.localdomain` (one HBA per fabric, named by both name servers,
zoned to nothing) — `present=False` on both fabrics, not in the array's `showhost`. The tool rendered one
zone per HBA (`g1-commands-F1.txt`, `-F2.txt`, 7 lines each, no warnings). With the training team's nod,
**F1 only** was pasted into SAN6700R13U38 exactly as generated:

```
cfgtransshow                                   -> There is no outstanding zoning transaction
alicreate "localhost_localdomain_hba2","10:00:5c:ed:8c:53:12:a3"
alicreate "CZ2D2K014S_N0S3P4","20:34:00:02:ac:02:d4:95"
zonecreate "localhost_localdomain_hba2_CZ2D2K014S_N0S3P4","localhost_localdomain_hba2;CZ2D2K014S_N0S3P4"
cfgadd "mycfg","localhost_localdomain_hba2_CZ2D2K014S_N0S3P4"
cfgsave                                        -> y ; Updating flash ...
cfgenable mycfg                                -> y ; zone config "mycfg" is in effect
```

`pending.ps1 -G1Check d45a12f6` re-ran Discovery and the zoning check: odd row `present=False → True
(10005CED8C5312A3)`; discovery shows the HBA logged into array port **0:3:4** — the port the tool chose
for the odd fabric under this rack's non-standard cabling (0:3:4 sits on the odd switch, 0:3:3 on the
even one); *Zoning needs 7 → 6 zone(s)*. The even row stayed `False` because F2 was not applied (the
script's one FAIL asserted it anyway). **The last unproven link in the provisioning track is closed:
a command set generated by the tool, applied by hand, made a host log in.** Undo lines are on file;
the zone stays unless the training team asks otherwise.
